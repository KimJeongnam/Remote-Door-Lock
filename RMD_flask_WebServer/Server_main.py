...
@app.route('/api/v1.0/Web/Service/DoorSwitch/4%6&*^90')
def Web_doorSwitch():
    if "userid" in session:
        id = session['userid']
        result = DoorSwitch(id)
...
@app.route('/api/v1.0/tasks/App/Service/DoorSwitch', methods=['POST'])
def App_Service_DoorSwitch():
    if request.method == 'POST':
        id = request.json['id']
        result = DoorSwitch(id)
        if result == 'Success':
            data = {'done':True,'id':'','password':'','name':''}
        else:
            data = {'done':False,'id':'','password':'','name':''}
    return jsonify(data)
